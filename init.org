...
(let* ((init-file (expand-file-name (concat user-emacs-directory "init.el")))
       (dt (format-time-string "%Y%m%d_%H%M%S" (current-time)))
       (init-file-backup (concat (file-name-sans-extension init-file) "_" dt ".el")))

  ;; copy init file to backup
  (when (file-exists-p init-file)
    (dired-copy-file init-file init-file-backup t)
    (message "Backup init.el created: %s" init-file-backup))
  
  ;; tangle file directly over original init file
...
(defun my/org-table-to-clipboard ()
  "Copy the Org table at point to the clipboard."
  (interactive)
  (when (org-at-table-p)
    (let ((table (cl-remove 'hline (org-table-to-lisp))))
      (with-temp-buffer
        (insert
         (mapconcat
          #'(lambda (row) (mapconcat 'identity row "\t"))
          table "\n"))
...
(defun my/org-copy-headline-number ()
  "Copy the current org-num-mode headline number to kill-ring."
  (interactive)
  (let* ((overlays (overlays-at (point)))
         (num-overlay (cl-find-if (lambda (o) (overlay-get o 'org-num)) overlays))
         (num (when num-overlay (substring-no-properties (overlay-get num-overlay 'after-string)))))
    (if num
        (progn
          (kill-new (string-trim num))
          (message "Copied: %s" (string-trim num)))
      (message "No org-num-mode number found at point."))))
#+end_src
...
;; As of Emacs 29.1 (2022) use-package was included in Emacs
(eval-when-compile
  ;; Following line is not needed if use-package.el is in the user-emacs-directory
  ;; (add-to-list 'load-path "<path where use-package is installed>")
  (require 'use-package))

(setopt use-package-always-ensure t)      ; ensure package is installed
(setopt use-package-verbose t)            ; report config loading time in *Messages*
#+END_SRC
...
(setq my/init-private-loaded
      (load
       (concat user-emacs-directory
               "init_private.el")))
;; defines: my/user-dir, my/on-work-pc, my/user-name, my/computer-name, my/org-directory, my/work-pc-agenda-files
#+END_SRC
...
(setq-default truncate-lines t)    ; truncate long lines, don't wrap them
(setopt column-number-mode t)        ; show column numbers in modeline
(setopt inhibit-startup-screen t)    ; skip startup screen
(show-paren-mode t)                ; highlight matching parentheses
(setopt show-paren-delay 0)
(setopt delete-pair-blink-delay 0)
(setopt show-paren-when-point-inside-paren t)
(setopt show-paren-when-point-in-periphery t)
(setopt show-paren-style 'parenthesis)    ; "mixed" and "expression" is far too obnoxious for incomplete expressions
;; (global-hl-line-mode t)            ; highlight current line
(setopt visible-bell t)              ; blink, don't bark
(setopt x-stretch-cursor t)          ; cursor width will match tab character width
(setq-default indent-tabs-mode nil)  ; indentation cannot use tabs, only space
...
(save-place-mode t)           ; return to last point in file
(setopt proced-enable-color-flag t)

;; backups - filename.ext~
(setopt backup-directory-alist `(("." . ,(expand-file-name ".backups/" user-emacs-directory)))) ; keep in clean
(setopt backup-by-copying t)              ; vs renaming

;; autosaves - #filename.ext#, when autosave mode enabled, saves unsaved changes
;; (setopt auto-save-list-file-prefix (expand-file-name ".auto-saves/" user-emacs-directory))

;; lock files - .#filename.ext
;; (setopt create-lockfiles nil)

(put 'narrow-to-region 'disabled nil)   ; enable arbitrary narrowing C-x n n

(toggle-scroll-bar 0)
(tool-bar-mode 0)                      ; Remove clunky toolbar and icons
(global-eldoc-mode 1)                  ; highlight parameters in minibuffer
(setopt reb-re-syntax 'string)         ; c-c TAB to cycle when in regexp-builder
...
(setopt delete-by-moving-to-trash t)   ; move files to trash instead of deleting

;; If current clipboard item from other program is smaller than 600 characters,
;; put it in killring before performing current emacs kill operation
(setopt save-interprogram-paste-before-kill 600)

(pixel-scroll-mode -1)                  ; legacy, ensure disabled
...
(setopt set-mark-command-repeat-pop t)   ; remove leading modifier key on repeat mark pops
...
;; Don't insert matching pair if cursor is on word-character, and other intelligent scenarios (this should be default)
(setopt electric-pair-inhibit-predicate 'electric-pair-conservative-inhibit)
;; If first character in parens is newline, insert additional newline so closing paren is on a separate line
...
;; Show buffer boundaries and if it extends past visible window in left fringe
(setopt indicate-buffer-boundaries '((top . left) (bottom . left) (up . left) (down . left)))

(setopt use-short-answers t)            ; y or n, instead of yes or no
(setopt confirm-kill-emacs 'yes-or-no-p) ; always confirm this

(setopt project-vc-extra-root-markers '(".dir-locals.el" "readme*" ".project")) ; Add C-x p p markers, not always in a vc directory
#+END_SRC
...
Some good programming fonts ([[https://blog.checkio.org/top-10-most-popular-coding-fonts-5f6e65282266?imm_mid=0f5f86][list]]).

1. Iosevka NF ← NerdFont patched
2. Inconsolata ← Nice, but no /italic/ or /oblique/? 😕
3. Fira Mono
4. Source Code Pro
5. Anonymous Pro
6. M+ 1M
...
(let ((default-font-name nil)
      (default-font-size 12))
  (cond
   ;; First choice
   ((find-font (font-spec :name "Iosevka NF"))
    (setq default-font-name "Iosevka NF")
    (setq default-font-size 13))
   
   ((find-font (font-spec :name "Inconsolata Nerd Font Regular"))
    (setq default-font-name "Inconsolata Nerd Font Regular")
    (setq default-font-size 13))
...
;; FRAMES
;; Set initial and default frame dynamically to be comfortable for primary display
(let* ((primary-monitor-disp (cdr (assoc 'geometry (car (display-monitor-attributes-list)))))
       (w (nth 2 primary-monitor-disp))
       (h (nth 3 primary-monitor-disp))
       (w-init (floor (* w .8)))
       (h-init (floor (* h .8))))

  ;; TODO: This should be done in early init: `.emacs.d/early-init.el'
  ;; Initial Frame
  (setq initial-frame-alist
        `((top . 10) (left . 1) (width . (text-pixels . ,w-init)) (height . (text-pixels . ,h-init))))
  ;; Default Frame
  (add-to-list 'default-frame-alist
               `(width . (text-pixels . ,w-init)))
  (add-to-list 'default-frame-alist
               `(height . (text-pixels . ,h-init)))
  )
#+END_SRC

*** Unicode Symbols and Emoji 🔥
When symbols or characters are displayed as boxes, Emacs is missing the font or configuration.
You can get access to many more icons, symbols, and even emoji with a bit of setup.

1. Use your system's included font for access to additional Unicode symbols.
   - http://xahlee.info/emacs/emacs/emacs_set_font_symbol.html
   - Windows: Segoe UI Symbol; Mac:Apple Symbols; Linux:Symbola

2. Install a patched font from the Nerd-Fonts downloads page and use that as your default.
   - https://www.nerdfonts.com/font-downloads
   - These are popular programming fonts that have been patched with additional glyphs related to programming.
   
3. Emoji (Emacs 29 or higher) provide access to additional symbols not in the unicode set.
   - https://fonts.google.com/noto/specimen/Noto+Emoji
   - [2026-07-03 Fri]: You CANNOT use color emoji in Emacs on Windows, use plain Noto Emoji.
     + https://www.reddit.com/r/emacs/comments/18t93lk/emojis_in_emacs_on_windows_10_solved/
   - For me, the light-bulb icon (💡) that =lsp-mode= defaults to for code-actions in the modeline wasn't in the Unicode or Nerd-Font... but is displayed after installing Noto Emoji.
   - Emoji Insert: =C-x 8 e e= 
...
#+BEGIN_SRC emacs-lisp
(defun my/theme-load-new (theme)
  "Disable any active themes and load THEME if it's available and not already enabled.
Return t if THEME was loaded, nil otherwise."
  (when (member theme (custom-available-themes))
    (unless (custom-theme-enabled-p theme)
      (mapc #'disable-theme custom-enabled-themes)
      (load-theme theme t))))

(defun my/theme-load-first-available (theme-list)
  "Load the first availalbe theme in THEME-LIST and return t.
If no theme in THEME-LIST is available, do nothing & return nil."
  (my/theme-load-new
   (cl-find-if
    (lambda (x)
      (member x (custom-available-themes)))
    theme-list)))


(defun my/list-cycle (thelist &optional prev)
  "Return a new list with the first element in THELIST moved to the end if PREV is nil.
Move the last item to the beginning of the list if PREV is non-nil."
  (progn
    (if (eq nil prev)
        ;; Cycle Next
        (setq thelist
      ...
      (setq thelist
            (append
             (last thelist)
             (take (1- (length thelist))
                   thelist))))))


(defun my/theme-cycle-dark (arg)
  "Cycle to the next dark theme in `my/themes-preferred-dark'. Cycle previous if prefix provided as `ARG'."
  (interactive "P")
  (let ((themelist my/themes-preferred-dark))
    (progn
      (if (eq nil arg)
          (setq themelist (my/list-cycle themelist))
        (setq themelist (my/list-cycle themelist t)))
      
      ;; update global list
      (setq my/themes-preferred-dark themelist)
      (message "Theme-Cycler (Dark) :: Loading Next: %s" (car themelist))
      (my/theme-load-first-available themelist))))


(defun my/theme-cycle-light (arg)
  "Cycle to the next light theme in `my/themes-preferred-light'. Cycle previous if prefix provided as `ARG'."
  (interactive "P")
  (let ((themelist my/themes-preferred-light))
    (progn
      (if (eq nil arg)
          (setq themelist (my/list-cycle themelist))
        (setq themelist (my/list-cycle themelist t)))
      ...
      ;; update global list
      (setq my/themes-preferred-light themelist)
      (message "Theme-Cycler (Light) :: Loading Next: %s" (car themelist))
      (my/theme-load-first-available themelist))))


(setq my/themes-preferred-dark '(two-fifteen modus-vivendi-tinted ef-owl ef-winter ef-night ef-deuteranopia-dark))
(setq my/themes-preferred-light '(modus-operandi-tinted leuven ef-deuteranopia-light ef-day))

(setq custom-safe-themes t)          ; show safety the door and load that theme!
(setq custom-theme-directory user-emacs-directory)
(my/theme-load-first-available my/themes-preferred-dark)
...
#+BEGIN_SRC emacs-lisp
(setenv "PYTHONIOENCODING" "utf-8")     ;print utf-8 in shell
...
(use-package writeroom-mode
  ...
  :defer t
  :config
  (setopt writeroom-restore-window-config t)
  )
#+end_src
...
- =lsp-mode= performance seems good since Emacs 27 JSON improvements.
  + setting the whitelist/blacklist of servers is useful for testing (=lsp-disabled-clients=, =lsp-enabled-clients=)
- =eglot= is an LSP client Emacs core since 29.1 (2022), but only supports a single server. [2025-11-28 Fri]
  + [[file:c:/Users/steven.brown/Apps/emacs-30.1/share/emacs/30.1/etc/NEWS.29::Protocol (LSP). See the new Info manual "(eglot) Top" for more. Also][eglot Info manual]]
...
(use-package lsp-mode
  ...
  :config
  (setopt lsp-modeline-code-action-fallback-icon "🗲")
  (setopt lsp-pylsp-plugins-autopep8-enabled t)  ; indent-region [ C-M-\ ] uses autopep8
  (setopt lsp-completion-provider :none)          ; autoconfigures to company-capf, even if company company-mode not installed
  (lsp-register-client                            ; Add Pyrefly as LSP client
   (make-lsp-client
    :new-connection (lsp-stdio-connection '("pyrefly" "lsp")) ; "-j" "4" : 4 threads instead of auto?
    :major-modes '(python-mode python-ts-mode)
    :server-id 'pyrefly
    :priority 1)))

;; Whitelist
;; (setq lsp-disabled-clients '(pylsp eslint)) ;; disable specific clients globally
;; Blacklist
;; (setq lsp-enabled-clients '(clangd rust-analyzer)) ;; allow only specific ones
...
#+begin_src emacs-lisp
(use-package eglot
  ;; :hook ((python-mode python-ts-mode) . eglot-ensure) ;; not enabled by default
  :config
...
(use-package doom-modeline
  :ensure t
  :init
  ;; before package has loaded
  :config
  ;; after doom-modeline has loaded
...
Improve Git performance on Windows! (magit uses many git calls to produce status screen)
...
#+BEGIN_SRC emacs-lisp
(setopt org-export-backends '(org md odt latex icalendar html ascii))
(setopt org-export-initial-scope 'subtree)
(setopt org-use-subsuperscripts '{})      ; require {} wrapper for ^super/_sub scripts
;; postamble
(setopt org-html-postamble t)
(setopt org-html-postamble-format
...
;; Add work specific agenda files if appropriate
(if (and (boundp 'my/work-pc-agenda-files) my/on-work-pc)
    (setq org-agenda-files (seq-uniq (append org-agenda-files my/work-pc-agenda-files))))
...
- [[https://orgmode.org/manual/Template-expansion.html][Org Template Expansion]]
...
(setq org-capture-templates
      `(("t"				; key
        ...
        ("l"
         ...
         ;; %(cond ((string-equal \"%K\" \"%a\" ) \"⏲↑%K\") (t \"⏲%K, ↑%a\")) → use only %K if both %K and %a are equal
         "1. %U %^{Log title|Today|Call w/X re Y}  [%(cond ((string-equal \"%K\" \"%a\" ) \"⏲↑%K\") (t (string-join '(\"⏲%K\" \"↑%a\") \", \")))]\n   - %?\n" ; [datetime] prompt  ⏲<link-to-currently-clocked-task>, ↑<link-to-current-header/location>
         :prepend nil
         :unnarrowed t
...
** Specifying the init-directory from the command-line
I like to have different shortcuts to Emacs that specify a different init directory. This allows me to try different Emacs configurations simply by creating a shortcut and changing the directory specified in the shortcut.

Apparently, this has been a command-line-option since emacs 29.
https://emacsredux.com/blog/2024/02/23/changing-the-emacs-configuration-directory/

Command-line options can be added to the shortcut in the Target field:
=C:\Path\to\emacs\bin\runemacs.exe --init-directory "C:/Path/to/specific/.emacs/"=

If the =init-directory= does not contain =init.el=, Emacs will fall back to other locations when searching for it. See "Inital Options" in the Emacs manual ([[info:emacs#Initial Options][Info]], [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Initial-Options.html#index-_002d_002dinit_002ddirectory][Web]]).

Previously, my shortcut launcher Target was much more complicated:
#+begin_example
C:\Path\to\emacs\bin\runemacs.exe
-q
--eval "(setq user-emacs-directory \"C:/Path/to/specific/.emacs/\")"
--load "C:\Path\to\specific\.emacs\init.el"
#+end_example

- -q :: don't load an init file
- --eval :: manually set the user-emacs-directory using elisp
- --load :: provide init file explicitly as generic elisp file to load

This was a messy hack that worked with caveats. Due to WHEN the eval code was executed in the init process, some environment elements will have already been set up using the default =.emacs.d= leading to an inconsistent environment that had to be corrected on a case-by-case basis. For example, my abbrevs (=abbrev-file-name=) wasn't using the desired path.

This /also/ meant that packages used by different configurations were all managed in the same directory (=package-user-dir=), and updating packages in one configuration would clobber them in another. Practically, this turned out to be not much of an issue for me, but /could/ have definitely introduced incompatibilities.
...
;; Plain text version of .dic file with just the words for autocompletion, ispell-lookup-words, cape-dict
(let ((dic-text (file-name-concat user-emacs-directory "en_CA.txt")))
  (when (file-exists-p dic-text)
    (setopt ispell-alternate-dictionary dic-text)
    (setopt cape-dict-file dic-text)))
#+END_SRC
...
(let* ((org-init-file (concat my/user-dir "Projects/dotemacs/init.org")) ; tied to home setup
  ...
  ;; ORG DIRECTORY FILES
  (when (and (stringp my/org-directory)
             (file-exists-p my/org-directory)
...
#+begin_src emacs-lisp
;; For graphical sessions, rebind C-z from suspend-frame, allows quick frame cycling
(when (display-graphic-p)
  (keymap-global-unset "C-z")
  (keymap-global-set "C-z" 'other-frame))
#+end_src
* Notes
